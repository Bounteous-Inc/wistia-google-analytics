...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    uglify: {
      ...
      build: {
        src: './src/lunametrics-wistia.gtm.js',
        dest: './lunametrics-wistia.gtm.min.js'
      }
    },
    fixConfig: {
      options: {
        build: {
          src: './lunametrics-wistia.gtm.min.js',
          dest: './lunametrics-wistia.gtm.min.js'
        }
      }
    },
    prependFooter: {
      options: {
        build: {
          src: './src/lunametrics-wistia.gtm.js',
          dest: './lunametrics-wistia.gtm.js'
        },
        footer: footer
      }
    },
    updateContainer: {
      options: {
        build: {
          src: './lunametrics-wistia.gtm.js',
          dest: './luna-wistia-tracking.json'
        }
      }
    }
  });
  ...
  grunt.registerTask('updateContainer', ['Updating container import file'], function() {
    ...
    for (i = 0; i < oldContainer.containerVersion.tag.length; i++) {
      ...
      if (oldContainer.containerVersion.tag[i].name === 'CU - Wistia Tracking - LunaMetrics Plugin') {
    ...
    oldContainer.containerVersion.tag[oldTag].parameter[oldParameter].value = '<script type="text/javascript" id="gtm-wistia-tracking">\n' +
      newScript +
      '\n</script>'
